...
#define TILE_SZ_A 16
static void dgemv_TransA(Concurrency::array_view<double> &A_mat, int aOffset,
...
static void dgemv_TransA_register(Concurrency::array_view<double> &A_mat, int aOffset,
                                  Concurrency::array_view<double> &X_vec, long xOffset,
                                  Concurrency::array_view<double> &Y_vec, long yOffset,
                                  double alpha, double beta, int lenX, int lenY,
                                  Concurrency::array_view<double> &tempBuf)
...
static void dgemv_NoTransA_register(Concurrency::array_view<double> &A, long aOffset,
...
static void dgemv_NoTransA(Concurrency::array_view<double> &A, long aOffset,
                           Concurrency::array_view<double> &X, long xOffset,
                           Concurrency::array_view<double> &Y, long yOffset,
                           double alpha, double beta, int lenX, int lenY)
{
...
void dgemv_AMP(char TransA, int M, int N, double alpha,
               ...
               Concurrency::array_view<double> &temp_buf)
{
    ...
    if (TransA == 't')
        dgemv_TransA_register(A, aOffset, X, xOffset, Y, yOffset, alpha, beta, lenX, lenY, temp_buf);
    else if (TransA == 'n')
        dgemv_NoTransA_register(A, aOffset, X, xOffset, Y, yOffset, alpha, beta, lenX, lenY);
}
ampblasStatus ampblas_sgemv(const enum AMPBLAS_TRANS type,
                              ...
                              double *Y, const long yOffset, const int incY)
{
    ...
    if (type == 'n')
    {
        ...
        dgemv_AMP(type, M, N, *alpha, aMat, aOffset, xView, xOffset, incX, *beta, yView, yOffset, incY, tempBuf);
        aMat.synchronize();
        /* Print Output */
        /*    for (int i = 0 ;i < M; i++) {
        cout << "[Y" << i << "] " << yView[i] << endl;
