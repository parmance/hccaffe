...
void Im2colLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
  ...
  for (int n = 0; n < bottom[0]->num(); ++n) {
    im2col_gpu2(bottom[0]->count()-bottom[0]->offset(n),top[0]->count()-top[0]->offset(n),bottom_data + bottom[0]->offset(n), channels_, height_,
...
void Im2colLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
  ...
  for (int n = 0; n < top[0]->num(); ++n) {
    ...
    col2im_gpu2(top[0]->count()-top[0]->offset(n),bottom[0]->count()-bottom[0]->offset(n),top_diff + top[0]->offset(n), channels_, height_, width_,
        kernel_h_, kernel_w_, pad_h_, pad_w_,
        stride_h_, stride_w_, bottom_diff + bottom[0]->offset(n));
  }
}
