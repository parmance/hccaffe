...
{
  array_view<float, 1> bottomDataView = *((Concurrency::array_view<float, 1>*)(bottom_data));
  array_view<float, 1> topDataView = *((Concurrency::array_view<float, 1>*)(top_data));
  bool maskTag = ( mask==NULL ? false:true);

  if (maskTag)
  {
    //     delte maskView;
    array_view<int, 1>  maskView=*((Concurrency::array_view<int, 1>*)(mask));
    parallel_for_each(
  ...
  {
    array_view<float, 1> topMaskView=*((Concurrency::array_view<float, 1>*)(top_mask));
    parallel_for_each(
      topDataView.get_extent(),
      [=](index<1> idx) restrict(amp)
    {
...
{
  array_view<double, 1> bottomDataView = *((Concurrency::array_view<double, 1>*)(bottom_data));
  array_view<double, 1> topDataView = *((Concurrency::array_view<double, 1>*)(top_data));
  bool maskTag = ( mask==NULL ? false:true);
  ...
  {
    array_view<double, 1>    topMaskView = *((Concurrency::array_view<double, 1>*)(top_mask));
    parallel_for_each(
      topDataView.get_extent(),
      [=](index<1> idx) restrict(amp)
    {
...
{
  array_view<float, 1> bottomDataView = *((Concurrency::array_view<float, 1>*)(bottom_data));
  array_view<float, 1> randIdxView = *((Concurrency::array_view<float, 1>*)(rand_idx));
  array_view<float, 1> topDataView = *((Concurrency::array_view<float, 1>*)(top_data));
  parallel_for_each(
    topDataView.get_extent(),
    [=](index<1> idx) restrict(amp)
...
{
  array_view<double, 1> bottomDataView = *((Concurrency::array_view<double, 1>*)(bottom_data));
  array_view<double, 1> randIdxView = *((Concurrency::array_view<double, 1>*)(rand_idx));
  array_view<double, 1> topDataView = *((Concurrency::array_view<double, 1>*)(top_data));
  parallel_for_each(
    topDataView.get_extent(),
    [=](index<1> idx) restrict(amp)
...
void MaxPoolBackward(int top_count, int boottom_count, float* top_diff,
                     ...
                     float* bottom_diff)
{
  array_view<float, 1> bottomDiffView = *((Concurrency::array_view<float, 1>*)(bottom_diff));
  array_view<float, 1> topDiffView = *((Concurrency::array_view<float, 1>*)(top_diff));
  bool maskTag = ( mask==NULL ? false:true);
...
void MaxPoolBackward(int top_count, int boottom_count, double* top_diff,
                     ...
                     double* bottom_diff)
{
  array_view<double, 1> bottomDiffView = *((Concurrency::array_view<double, 1>*)(bottom_diff));
  array_view<double, 1> topDiffView = *((Concurrency::array_view<double, 1>*)(top_diff));
  bool maskTag = ( mask==NULL ? false:true);
